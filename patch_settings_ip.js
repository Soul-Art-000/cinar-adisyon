...
const ipState = `  const [localIp, setLocalIp] = useState('');
  useEffect(() => {
    if (window.__TAURI__) {
...
code = code.replace("export default function SettingsAdmin({ zones, categories, onAddZone, onDeleteZone, onAddCategory, onDeleteCategory }) {", "export default function SettingsAdmin({ zones, categories, onAddZone, onDeleteZone, onAddCategory, onDeleteCategory }) {\n" + ipState);

const ipUI = `      {localIp && (
        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-6 shadow-sm mb-8">
...
code = code.replace(/<div className="max-w-4xl mx-auto space-y-8">/, '<div className="max-w-4xl mx-auto space-y-8">\n' + ipUI);

fs.writeFileSync('adisyon-app/src/components/SettingsAdmin.jsx', code);
console.log("Settings IP patched.");
